...
parameters { 
    cholesky_factor_corr[2+2*N_cascadelevel_features] L_z; // mixture probabilities, set first class to 0, order is 0+0x, 1+1x
    ...
    vector[2+2*N_cascadelevel_features] beta_a_z_raw;
    vector[2] beta_a_plfast_raw;
    vector[2] beta_a_plmid_raw;
    vector[2] beta_a_plslow_raw;

    array[V_train] vector<lower=-5,upper=5>[2+2*N_cascadelevel_features] beta_av_z_raw;
    ...
    vector<lower=0>[2+2*N_cascadelevel_features] sigma_beta_a_z;
...
transformed parameters { 
    ...
    vector[2+2*N_cascadelevel_features] beta_a_z_mean = rep_vector(0, 2+2*N_cascadelevel_features);
    beta_a_z_mean[1] = -2; // slow intercept
    beta_a_z_mean[2+N_cascadelevel_features] = -2; // fast intercept

    vector[2+2*N_cascadelevel_features] beta_a_z_sd = rep_vector(0.1, 2+2*N_cascadelevel_features);
    beta_a_z_sd[1] = 1; // slow intercept
    beta_a_z_sd[2+N_cascadelevel_features] = 1; // fast intercept
    ...
    vector[2+2*N_cascadelevel_features] beta_a_z = beta_a_z_mean + beta_a_z_sd .* beta_a_z_raw;
    ...
    array[V_train] vector[2+2*N_cascadelevel_features] beta_av_z;
...
model {
    ...
    sigma_beta_a_z[2:1+N_cascadelevel_features] ~ normal(0,0.1); // slow magnitude effect
    sigma_beta_a_z[2+N_cascadelevel_features] ~ normal(0,1); // fast intercept
    sigma_beta_a_z[3+N_cascadelevel_features:] ~ normal(0,0.1); // fast magnitude effect
    ...
    for (v in 1:V_train) {        
        ...
        for (k in 1:C_of_v) {
            ...
            vector[N_cascadelevel_features+1] x_avc = append_row(1, cascadelevel_features_train[(pos_C+k-1-1)*N_cascadelevel_features+1:(pos_C+k-1)*N_cascadelevel_features]);

            vector[3] z_to_raise = rep_vector(0, 3);
            z_to_raise[2] = gamma_effect_on_slow + dot_product(beta_av_z[v][1:1+N_cascadelevel_features], x_avc); //slow
            z_to_raise[3] = gamma_effect_on_fast + dot_product(beta_av_z[v][2+N_cascadelevel_features:2+2*N_cascadelevel_features], x_avc); //fast
...
generated quantities {
    ...
    array[V_holdout] vector[2+2*N_cascadelevel_features] beta_av_z_holdout;
    ...
    matrix[2+2*N_cascadelevel_features,2+2*N_cascadelevel_features] Omega_beta_z = L_z * L_z';
    ...
    matrix[2+2*N_cascadelevel_features,2+2*N_cascadelevel_features] covariance_beta_z = quad_form_diag(Omega_beta_z, sigma_beta_a_z);
    ...
    for (v in 1:V_holdout) {        
        ...
        for (k in 1:C_of_v) {
            ...
            vector[N_cascadelevel_features+1] x_avc_holdout = append_row(1, cascadelevel_features_holdout[(pos_C+k-1-1)*N_cascadelevel_features+1:(pos_C+k-1)*N_cascadelevel_features]);

            vector[3] z_to_raise = rep_vector(0, 3);
            z_to_raise[2] = gamma_effect_on_slow_holdout+ dot_product(beta_av_z_holdout[v][1:1+N_cascadelevel_features], x_avc_holdout); //slow
            z_to_raise[3] = gamma_effect_on_fast_holdout + dot_product(beta_av_z_holdout[v][2+N_cascadelevel_features:2+2*N_cascadelevel_features], x_avc_holdout); //fast
