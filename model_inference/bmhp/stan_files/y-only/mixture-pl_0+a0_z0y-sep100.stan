...
parameters { 
    ...
    vector[3] beta_a_raw; // intercept z , intercept_pop, intercept_unpop, covariate_z
...
model {
    ...
    gamma_a_alpha_sd ~ exponential(100); // prior for laplace scale
    ...
    for (v in 1:V_train) {        
        ...
        for (k in 1:C_of_v) {
            ...
            real zinf_avc = inv_logit(beta_av[v][1] + dot_product(gamma_a_alpha, y_av));
...
generated quantities {
    ...
    for (v in 1:V_holdout) {      
        ...
        for (k in 1:C_of_v) {
            real zinf_avc_holdout = inv_logit(beta_av_holdout[v][1] + dot_product(gamma_a_alpha, y_av_holdout));
