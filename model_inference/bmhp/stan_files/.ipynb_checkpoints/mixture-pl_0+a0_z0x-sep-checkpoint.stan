...
parameters { 
    ...
    vector[N_cascadelevel_features + 3] beta_a_raw; // intercept_pop, intercept_unpop, intercept_z, covariate_z
...
transformed parameters {
    ...
    beta_a_mean[1] = -1.39; // bias the popular probability
...
model {
    ...
    for (v in 1:V_train) {        
        ...
        real alpha_avc_popular = inv_logit(beta_av[v][2]);
        real alpha_avc_unpopular = inv_logit(beta_av[v][3]);
        ...
        for (k in 1:C_of_v) {
            ...
            real zinf_avc = inv_logit(beta_av[v][1] + dot_product(beta_av[v][4:], x_avc));
...
generated quantities {
    ...
    vector[C_holdout] pred_cascadesize_holdout = rep_vector(0,C_holdout);
    ...
    for (v in 1:V_holdout) {      
        ...
        real alpha_avc_popular_holdout = inv_logit(beta_av_holdout[v][2]);
        real alpha_avc_unpopular_holdout = inv_logit(beta_av_holdout[v][3]);

        for (k in 1:C_of_v) {
            vector[N_cascadelevel_features] x_avc_holdout = cascadelevel_features_holdout[(pos_C+k-1-1)*N_cascadelevel_features+1:(pos_C+k-1)*N_cascadelevel_features];
            real zinf_avc_holdout = inv_logit(beta_av_holdout[v][1] + dot_product(beta_av_holdout[v][4:], x_avc_holdout));
            ...
            log_lik[v] += log_sum_exp(log(zinf_avc_holdout) + poisson_lpmf(cascade_sizes_holdout[k]-1 | cascade_sizes_holdout[k]*alpha_avc_popular_holdout) - log(cascade_sizes_holdout[k]), log1m(zinf_avc_holdout) + poisson_lpmf(cascade_sizes_holdout[k]-1| cascade_sizes_holdout[k]*alpha_avc_unpopular_holdout) - log(cascade_sizes_holdout[k]));
            
            pred_cascadesize_holdout[pos_C+k-1] = pred_cascadesize_avc_holdout;
        }
